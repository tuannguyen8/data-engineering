...
#line 155			ALTER TABLE {TableName} ADD PRIMARY KEY (TractId);
#line 156			CREATE INDEX idx_{TableName}_State ON {TableName}(State);
...
def load(conn, csv_path):
    with conn.cursor() as cursor:
        print(f"Loading data from {csv_path} using COPY...")
        start = time.perf_counter()

        with open(csv_path, 'r') as f:
            next(f)  # Skip the header row
            cursor.copy_from(f, TableName, sep=',', null='')  # default is null='\\N'

        conn.commit()
        elapsed = time.perf_counter() - start
        print(f'Finished Loading with copy_from. Elapsed Time: {elapsed:.4f} seconds')
...
def addConstraintsAndIndexes(conn):
    with conn.cursor() as cursor:
        ...
        """)
        print("Primary key and index added.")
